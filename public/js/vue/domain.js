...
const RefinementList = Vue.extend({
  ...
  ready() {
    $algoliasearchHelper.on('result', results => {
      this.facets = results.getFacetValues('domain', ['selected', 'count:desc']).slice(0, 5);
    });
  },

  methods: {
    toggleFacet(facetName) {
      $algoliasearchHelper.toggleRefinement('domain', facetName).search();
    }
  }
});
